...
class ContaReceber extends model {
    ...
    public function getList($s = null, $offset, $limit) {
        ...
        if (!empty($s)) {
            $sql = $this->db->prepare("SELECT contas_receber.id, cliente.nome,cliente.cpfCnpj, venda.data_venda, venda.total_venda, contas_receber.data_vencimento,contas_receber.data_recebimento,contas_receber.status
                   FROM contas_receber
                   ...
                   ORDER BY contas_receber.status = 0
                   LIMIT $offset,$limit");
    ...
    public function getTotal($s) {
        if (!empty($s)) {
            $sql = $this->db->prepare("SELECT COUNT(*)as c FROM contas_receber WHERE nome LIKE :nome");
            $sql->bindValue(":nome", '%' . $s . '%');
            $sql->execute();
        } else {
    ...
    public function getInfo($id) {
        ...
        $sql = $this->db->prepare("SELECT cliente.nome,cliente.cpfCnpj,venda.data_venda,
               venda.total_venda,contas_receber.data_vencimento,contas_receber.status,contas_receber.dinheiro,contas_receber.troco
               FROM contas_receber
               INNER JOIN venda on venda.id = contas_receber.id_venda
               INNER JOIN cliente on cliente.id = venda.id_cliente
