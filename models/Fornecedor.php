...
class Fornecedor extends model
{
    ...
    public function getList($s = null, $offset, $limit)
    {
        ...
        if (!empty($s)) {
            $sql = $this->db->prepare("SELECT fornecedor.id,fornecedor.razao_social,fornecedor.cnpj,fornecedor.telefone,cidade.nome as cidade, estado.nome as estado
            FROM fornecedor
            INNER JOIN cidade on cidade.id = fornecedor.id_cidade
    ...
    public function getFornecedor($id)
    {
        ...
        $sql = $this->db->prepare("SELECT cidade.id_estado, fornecedor.*
        FROM fornecedor
        INNER JOIN cidade on cidade.id= fornecedor.id_cidade
    ...
    public function fornecedor_add($razao_social, $nome_fantasia, $cnpj, $ie, $telefone, $data_cadastro, $cep, $bairro, $rua, $numero, $cidade)
    {

        $sql = $this->db->prepare("INSERT INTO fornecedor(razao_social,nome_fantasia,cnpj,ie,telefone,data_cadastro,cep,bairro,rua,numero,id_cidade)
        VALUES(:razao_social,:nome_fantasia,:cnpj,:ie,:telefone,:data_cadastro,:cep,:bairro,:rua,:numero,:cidade)");
    ...
    public function fornecedor_editar($razao_social, $nome_fantasia, $cnpj, $ie, $telefone, $data_cadastro, $cep, $bairro, $rua, $numero, $cidade, $id)
    {
        $sql = $this->db->prepare("UPDATE fornecedor SET razao_social = :razao_social, nome_fantasia = :nome_fantasia, cnpj = :cnpj, ie = :ie, telefone = :telefone, data_cadastro = :data_cadastro, cep = :cep, bairro = :bairro, rua = :rua, numero = :numero, id_cidade = :cidade WHERE id = :id");
        $sql->bindValue(":razao_social", $razao_social);
        $sql->bindValue(":nome_fantasia", $nome_fantasia);
        $sql->bindValue(":cnpj", $cnpj);
        $sql->bindValue(":ie", $ie);
    ...
    public function getIdEstado($id)
    {

        $sql = $this->db->prepare("SELECT cidade.id_estado as id
        FROM fornecedor 
        INNER JOIN cidade on cidade.id = fornecedor.id_cidade
        WHERE fornecedor.id = :id");
        $sql->bindValue(":id", $id);
